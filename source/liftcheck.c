...
unsigned char TDATA[64][22] = { { 0x7B, 0x00, 0x00, 0x00, 0x00, 0x00, 0x0E, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x7D },
...
int process_server1(int *SocketConnect, char* SocketRxBuf, int SocketRxlen)
{
	...
	if ((SocketRxBuf[0] == 0x7B) && (SocketRxBuf[2] == 0x00))//02: Dongzhi
	{
		...
		if ((SocketRxBuf[SUP] & bit1) == 0)
		{
			...
			{
				pElevatorStatus.EStatus_speed = 1000 * UpDownDistance / timeIntervalUsec;//10^6-->10^5
...
{
	return GPIORead(S_POWER_STATUS);//GPC14(RGB_VD[6])?-->GPD2
}
...
void refreshStatus(void)
{
	...
	char testData1[13] = { 0x7B, 0x00, 0x02, 0x01, 0x00, 0x00, 0x05, 0x00, 0x80, 0x00, 0x00, 0x03, 0x7D };//dongzhi
	//char testData3[11]= {0x7B,0x00,0x01,0x00,0x00,0x00,0x03,0x0F,0xE0,0x01,0x7D};//sanrong
	//process_server1((int*)0, testData1, 13);
	//process_server3((int*)0,testData3,11);
	getElevatorStatus_IO1();
	//getElevatorStatus_IO2();
	//getElevatorStatus_IO3();
...
void ioData2_init(void)
{
	...
	pIOData2.IOData2_direction = 0;//012		//#3??DD·??ò
	pIOData2.IOData2_carDoorStatus = 2;//012	//#4?á???a±?
	pIOData2.IOData2_powerCut = 0;//01		//#5μ?ìYí￡μ?
	pIOData2.IOData2_stopOutArea = 0;//01		//#6????íaí￡ìY
	pIOData2.IOData2_hitCeiling = 0;//01		//#73??￥
	pIOData2.IOData2_hitGround = 0;//01		//#8?×μ×
	pIOData2.IOData2_openWhileRunning = 0;//01	//#9??DD?D?a??
	pIOData2.IOData2_overSpeed = 0;//01		//#103??ù
	pIOData2.IOData2_openWithoutStopping = 0;//01	//#11?a??×?ìY
	pIOData2.IOData2_peopleTrapped = 0;//01		//#12μ?ìYà§è?
	pIOData2.IOData2_absoluteFloor = 1;		//#13?????￥2?
}
...
void checkElevatorFault_IO1(void)
{
	...
	int errorCode = pIOData1.IOData1_errorCode;
	...
	case 0x06:
		...
		break;
	case 0x07:
		//DEBUG("Fault: A_LOCK_LOOP_BREAK\n");
		DEBUG("Fault: A_OPEN_WHILE_RUNNIN\n");
		//setFault(A_LOCK_LOOP_BREAK);
		setFault(A_OPEN_WHILE_RUNNIN);
		break;
	case 0x09:
		//DEBUG("Fault: A_SPEED_ANOMALY\n");
		DEBUG("Fault: A_OVERSPEED\n");
		//setFault(A_SPEED_ANOMALY);
...
int httpPost_DeviceRegister(void)
{
	...
	Cat1_return_t ret;
	memset(httpbuf, 0, 1024);
	sprintf(httpbuf, "#10#{\"type\":\"login\",\"eid\":\"mx12345678\"}#13#\0");

	ret = cat1_send_raw(httpbuf,strlen(httpbuf));
...
void liftCheckTask(void *pvParameters)
{
	...
	while (1)
	{
		...
		vTaskDelay((portTickType) UINT32_C(400) / portTICK_RATE_MS);
#if 0
		...
		vTaskDelay((portTickType) UINT32_C(1000) / portTICK_RATE_MS);
#endif
	}
	return;
